...
export function VariantEditForm({ variant, productCode, lightTones }: VariantEditFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    includes_led: variant.includes_led || false,
  ...
  const [selectedLightTones, setSelectedLightTones] = useState<number[]>(
    variant.variant_light_tones?.map((vlt: any) => vlt.light_tone.id) || []
  )

  // Configuraciones
  const [configurations, setConfigurations] = useState<Configuration[]>(
    variant.variant_configurations || []
  )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Actualizar datos básicos de la variante
      const response = await fetch(`/api/products/${productCode}/variants/${variant.id}`, {
        method: 'PATCH',
      ...
      if (newImages.cover) {
        uploadPromises.push(
          uploadImage(newImages.cover, 'cover')
        )
      }

      if (newImages.tech) {
        uploadPromises.push(
          uploadImage(newImages.tech, 'tech')
        )
      }
      ...
      console.error(error)
      alert('Error al actualizar la variante: ' + (error instanceof Error ? error.message : 'Error desconocido'))
    } finally {
      setIsSaving(false)
    }
  }
  ...
  const uploadImage = async (file: File, kind: 'cover' | 'tech' | 'datasheet' | 'spec' | 'gallery') => {
    ...
    formData.append('altText', `${kind} de ${variant.name}`)
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setFormData({ ...formData, variant_code: e.target.value.replace(/\s+/g, '-') })}
              className="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-theme-sm text-gray-900 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-500 focus:outline-none focus:ring-4 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-800 dark:text-white dark:placeholder:text-gray-500"
            />
          </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div className="p-6 space-y-6">
          {/* Imagen Cover */}
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">Imagen de Portada</h3>
            {existingImages.cover && !newImages.cover ? (
              <div className="relative inline-block">
                ...
                <button
                  ...
                  className="absolute -right-2 -top-2 rounded-full bg-error-500 p-1.5 text-white shadow-lg hover:bg-error-600"
                >
              ...
              <LocalImageUpload
                ...
                description="Selecciona una nueva imagen de portada"
                file={newImages.cover || null}
          ...
          {/* Imagen Tech */}
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">Imagen Técnica</h3>
            {existingImages.tech && !newImages.tech ? (
              <div className="relative inline-block">
                ...
                <button
                  type="button"
                  onClick={() => handleDeleteImage('tech')}
                  className="absolute -right-2 -top-2 rounded-full bg-error-500 p-1.5 text-white shadow-lg hover:bg-error-600"
                >
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => handleConfigChange(index, 'length_cm', e.target.value || undefined)}
                      placeholder="Ej: 600, 30x60, etc."
                      className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => handleConfigChange(index, 'width_cm', e.target.value || undefined)}
                      placeholder="Ej: 300, 30x30, etc."
                      className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
                    />
                  </div>
                </div>
